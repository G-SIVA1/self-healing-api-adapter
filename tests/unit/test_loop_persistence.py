from __future__ import annotations

from datetime import datetime, timezone
from pathlib import Path

import pytest

from self_healing_agent.agent.architect import CodePatch
from self_healing_agent.agent.context import ReasoningContextBuilder
from self_healing_agent.agent.decision import RetryDecisionEngine
from self_healing_agent.agent.history import RepairHistory
from self_healing_agent.agent.history_store import RepairHistoryStore
from self_healing_agent.agent.loop import SelfCorrectionLoop
from self_healing_agent.agent.models import RepairReasoning
from self_healing_agent.agent.state import AgentState
from self_healing_agent.models.events import ErrorEvent


class FakeReasoner:
    async def reason(self, context):
        if context.current_source_code is None:
            raise ValueError("Current source code is required")

        patched_source = context.current_source_code.replace(
            "stripe.Customer.create",
            "stripe.customers.create",
            1,
        )

        patch = CodePatch(
            file_path=context.target_file,
            original_code=context.current_source_code,
            replacement_code=patched_source,
            explanation="Replace deprecated Stripe API.",
        )

        return RepairReasoning(
            explanation="Replace deprecated Stripe API.",
            confidence=0.95,
            patch=patch,
        )


class FakeRepairExecutor:
    async def execute(
        self,
        patch,
        workspace: Path,
        validation_command: list[str],
        state: AgentState,
    ):
        target_file = workspace / patch.file_path

        target_file.write_text(
            patch.replacement_code,
            encoding="utf-8",
        )

        state.record_test_result(
            passed=True,
            output="1 passed",
        )

        state.repair_complete = True

        return object()


def create_state() -> AgentState:
    event = ErrorEvent(
        timestamp=datetime.now(timezone.utc),
        exception_type="AttributeError",
        message="module 'stripe' has no attribute 'Customer'",
        file_path="examples/broken_api_client.py",
        line_number=5,
        function_name="create_customer",
        api_service="stripe",
        api_call="stripe.Customer.create",
        raw_log=(
            "AttributeError: module 'stripe' "
            "has no attribute 'Customer'"
        ),
    )

    return AgentState(
        error_event=event,
        max_iterations=3,
    )


def create_workspace(tmp_path: Path) -> None:
    target_file = (
        tmp_path
        / "examples"
        / "broken_api_client.py"
    )

    target_file.parent.mkdir(
        parents=True,
        exist_ok=True,
    )

    target_file.write_text(
        "import stripe\n\n"
        "def create_customer(email):\n"
        "    return stripe.Customer.create(email=email)\n",
        encoding="utf-8",
    )


def create_loop(
    history: RepairHistory,
    history_store: RepairHistoryStore,
) -> SelfCorrectionLoop:
    return SelfCorrectionLoop(
        architect=None,
        repair_executor=FakeRepairExecutor(),
        reasoner=FakeReasoner(),
        context_builder=ReasoningContextBuilder(),
        decision_engine=RetryDecisionEngine(),
        history=history,
        history_store=history_store,
    )


@pytest.mark.asyncio
async def test_loop_persists_successful_repair(
    tmp_path: Path,
) -> None:
    create_workspace(tmp_path)

    database_path = tmp_path / "history.db"

    history = RepairHistory()
    history_store = RepairHistoryStore(database_path)

    loop = create_loop(
        history=history,
        history_store=history_store,
    )

    result = await loop.run(
        state=create_state(),
        workspace=tmp_path,
        validation_command=[
            "python",
            "-m",
            "pytest",
            "-q",
        ],
    )

    assert result.succeeded is True
    assert history.count == 1
    assert history_store.count() == 1

    attempts = history_store.list_attempts()

    assert len(attempts) == 1

    attempt = attempts[0]

    assert attempt.status == "success"
    assert attempt.test_passed is True
    assert attempt.confidence == 0.95

    assert "stripe.Customer.create" in attempt.original_code
    assert "stripe.customers.create" in attempt.generated_code


@pytest.mark.asyncio
async def test_persisted_history_survives_new_store(
    tmp_path: Path,
) -> None:
    create_workspace(tmp_path)

    database_path = tmp_path / "history.db"

    first_store = RepairHistoryStore(database_path)

    loop = create_loop(
        history=RepairHistory(),
        history_store=first_store,
    )

    result = await loop.run(
        state=create_state(),
        workspace=tmp_path,
        validation_command=[
            "python",
            "-m",
            "pytest",
            "-q",
        ],
    )

    assert result.succeeded is True

    second_store = RepairHistoryStore(database_path)

    assert second_store.count() == 1

    attempts = second_store.list_attempts()

    assert len(attempts) == 1
    assert (
        attempts[0].target_file
        == "examples/broken_api_client.py"
    )
    assert attempts[0].status == "success"


@pytest.mark.asyncio
async def test_in_memory_and_persistent_history_stay_in_sync(
    tmp_path: Path,
) -> None:
    create_workspace(tmp_path)

    database_path = tmp_path / "history.db"

    history = RepairHistory()
    history_store = RepairHistoryStore(database_path)

    loop = create_loop(
        history=history,
        history_store=history_store,
    )

    await loop.run(
        state=create_state(),
        workspace=tmp_path,
        validation_command=[
            "python",
            "-m",
            "pytest",
            "-q",
        ],
    )

    assert history.count == 1
    assert history_store.count() == 1

    in_memory_attempt = history.latest

    assert in_memory_attempt is not None

    persisted_attempt = history_store.list_attempts()[0]

    assert (
        persisted_attempt.iteration
        == in_memory_attempt.iteration
    )

    assert (
        persisted_attempt.target_file
        == in_memory_attempt.target_file
    )

    assert (
        persisted_attempt.test_passed
        == in_memory_attempt.test_passed
    )

    assert (
        persisted_attempt.generated_code
        == in_memory_attempt.generated_code
    )

    assert (
        persisted_attempt.status
        == in_memory_attempt.status
    )